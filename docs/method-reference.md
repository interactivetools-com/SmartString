...
| `SmartString::getRawValue($value)` | Returns the original value when you don't know what you have: Smart* objects are converted to their original value, plain values pass through unchanged |

### [Encoding](encoding-and-html.md)

*These return the encoded value as a plain string, so they end the chain. Missing values (null or "") return "", so echoing an empty field prints nothing, except where noted below.*

| Method                        | Description                                                                                                                         |
|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| `->htmlEncode()`              | Returns the value as an HTML-encoded string                                                                                         |
| `->urlEncode()`               | Returns the value as a URL-encoded string                                                                                           |
| `->jsonEncode()`              | Returns the value as a JSON-encoded string; missing values stay valid JSON (null → `null`, "" → `""`), malformed UTF-8 becomes � instead of throwing |
...
| `->divide($value)`                         | Divides the current number by `$value`                                                               |
